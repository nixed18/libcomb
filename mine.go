...
func miner_mine_commit_internal(rawcommit [32]byte, tag utxotag) {
	...
	if direction_mine_unmine == UTAG_END_OF_BLOCK {
		...
		if len(commit_rollback) > 0 && len(commit_cache) > 0 {
			log("bidirectional block not supported")
			goto adios
	...
	if commitment_mining_selfprinting {
		//logf("wget http://127.0.0.1:2121/mining/mine/%X/%X -O /dev/null\n", rawcommit, rawutxotag)
	}
