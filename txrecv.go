...
func tx_receive_transaction_internal(transaction [763]byte) [2][32]byte {
	...
	if txcommitsandfrom[21] == txidandto[1] {
		logf("warning: transaction forms a trivial loop from to: %X", txidandto[1])
	}
	...
	if actuallyfrom != txcommitsandfrom[21] {
		logf("error: transaction is from user: %X", actuallyfrom)
